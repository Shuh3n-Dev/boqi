...
function FeatureCard({ feature }: { feature: (typeof features)[number] }) {
  return (
    <motion.div
      variants={itemAnim}
      className={`col-span-full ${feature.colSpan} ${feature.bg} px-lg py-lg rounded-lg flex flex-col justify-between transition-all duration-300 group`}
    >
      <div>
        <div
        ...
        <h3 className="text-headline-sm text-headline-sm mb-sm">
          {feature.title}
        </h3>
        <p
          className={`text-body-md text-body-md text-on-surface-variant max-w-lg leading-relaxed ${feature.textColor}`}
        >
          {feature.description}
        </p>
      </div>

      <div className="flex items-end justify-between mt-lg pt-md border-t border-outline-variant/30">
        {feature.link && (
          <span className="text-body-sm text-body-sm text-on-surface-variant/60 flex items-center gap-sm group-hover:text-accent transition-colors">
            Saber más
    ...
    <section className="w-full px-gutter mb-xl scroll-mt-24" id="features">
      <div className="w-[70%] mx-auto max-w-[1280px]">
        ...
        >
          ...
          <p className="text-body-md text-body-md text-on-surface-variant mt-sm max-w-xl">
            Cada componente diseñado para eliminar fricción, optimizar tiempos y
            darle control total sobre su operación.
          </p>
          <div className="h-1 w-16 bg-gradient-to-r from-primary via-primary-container to-accent mt-sm rounded" />
        </motion.div>
